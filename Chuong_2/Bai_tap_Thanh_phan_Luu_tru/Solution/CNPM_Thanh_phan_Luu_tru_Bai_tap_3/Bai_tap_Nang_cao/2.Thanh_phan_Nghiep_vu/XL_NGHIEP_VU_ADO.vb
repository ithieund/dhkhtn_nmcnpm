...
#Region "Trích rút thông tin"
    ...
    Public Function Danh_sach_Ngoai_ngu(Du_lieu As DataSet) As List(Of DataRow)
        Dim kq As List(Of DataRow) = Du_lieu.Tables("NGOAI_NGU").Select.ToList
        Return kq
    End Function
    ...
    Public Function Danh_sach_Nhan_vien_theo_Do_tuoi(Du_lieu As DataSet, Can_duoi As Double, Can_tren As Double) As List(Of DataRow)
        Dim Kq As List(Of DataRow)
        Kq = Danh_sach_Nhan_vien(Du_lieu).FindAll(Function(x) Today.Year - Date.Parse(x("Ngay_sinh")).Year >= Can_duoi And Today.Year - Date.Parse(x("Ngay_sinh")).Year <= Can_tren)
        Return Kq
    End Function
#End Region
