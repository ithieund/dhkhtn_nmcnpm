...
#Region "Xu ly trich rut"
    Public Function Danh_sach_Nhan_vien_theo_Muc_luong(Du_lieu As DataSet, Muc_luong_Min As Integer, Muc_luong_Max As Integer) As List(Of DataRow)

        Dim Kq As List(Of DataRow)
        Dim Bang_Nhan_vien As DataTable = Du_lieu.Tables("NHAN_VIEN")
        Dim Dieu_kien As String = "Muc_luong >= " & Muc_luong_Min & " AND Muc_luong <= " & Muc_luong_Max
...
#Region "Tiep nhan thong tin"
    ...
    Public Function Kiem_tra_Bo_loc(Muc_luong_Min As String, Muc_luong_Max As String) As String

        Dim Muc_luong_Min_thuc As Integer
        Dim Muc_luong_Max_thuc As Integer

        Dim Kq As String = ""
        Dim Hop_le As Boolean = True

        Hop_le = Integer.TryParse(Muc_luong_Min, Muc_luong_Min_thuc)

        If Not Hop_le Then
            Kq &= "Mức lương min phải là số nguyên" & vbCrLf
        Else
            Hop_le = Muc_luong_Min_thuc > 0

            If Not Hop_le Then
                Kq &= "Mức lương min phải là số dương" & vbCrLf
            End If
        End If

        Hop_le = Integer.TryParse(Muc_luong_Max, Muc_luong_Max_thuc)

        If Not Hop_le Then
            Kq &= "Mức lương max phải là số nguyên" & vbCrLf
        Else
            Hop_le = Muc_luong_Max_thuc > 0

            If Not Hop_le Then
                Kq &= "Mức lương max phải là số dương" & vbCrLf
            End If
        End If

        If Kq = "" Then
            Hop_le = Muc_luong_Min_thuc <= Muc_luong_Max_thuc

            If Not Hop_le Then
                Kq &= "Mức lương min phải nhỏ hơn hoặc bằng mức lương max" & vbCrLf
            End If
        End If
